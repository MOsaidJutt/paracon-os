...
import { setOrgConfig, resetOrgConfig } from "@/lib/config";
...
export async function PATCH(req: NextRequest, { params }: { params: { key: string } }) {
  try {
    ...
    await setOrgConfig(session.user.organisationId, params.key, parsed.data, session.user.id);
...
export async function DELETE(_req: NextRequest, { params }: { params: { key: string } }) {
  try {
    const session = await requirePermission("admin.settings");
    await resetOrgConfig(session.user.organisationId, params.key);
