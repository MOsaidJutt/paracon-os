...
import { querySupplierBill } from "@/lib/finance/bill-review-service";
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const bill = await querySupplierBill(db, session.user.organisationId, session.user.id, params.id, body.note);
